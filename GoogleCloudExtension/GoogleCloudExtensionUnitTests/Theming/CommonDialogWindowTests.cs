...
namespace GoogleCloudExtensionUnitTests.Theming
{
    ...
    public class CommonDialogWindowTests : WpfTestBase<CommonDialogWindow<ICloseSource>>
    {
        ...
        {
            _contentMock = new Mock<ICommonWindowContent<ICloseSource>>();
        ...
        public void TestConstructor_RegistersClose()
        {
            bool? showModalResult = GetResult(
                w => _contentMock.Raise(c => c.Close += null),
        ...
        [TestMethod]
        public void TestOnClose_CallsContentOnParentClose()
        {
            GetResult(
                w => w.Close(),
                () => _objectUnderTest.ShowModal());

            _contentMock.Verify(c => c.OnParentClosed());
        }
